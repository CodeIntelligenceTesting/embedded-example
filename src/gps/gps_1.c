...
enum GPS_return_status init_crypto_module() {
  crypto_init();
  crypto_key key = {0};
  key_management_create_key(key.key, sizeof(key.key));
  if (crypto_set_key(key) == valid_key_provided) {
    crypto_nonce nonce = {0};
    key_management_create_nonce(nonce.nonce, sizeof(nonce));
...
enum GPS_return_status get_current_position(GPS_position *position) {
  ...
  crypto_hmac hmac_as_bytes = {0};
  if (GPS_driver_obtain_current_position(position_as_bytes, hmac_as_bytes.hmac) ==
      0) {
